...
router.post('/projects', requireAdmin, (req, res) => {
  ...
  const { title, description = '', cover = '', tags = [], category = '工具', githubUrl = '', demoUrl = '', featured = 0 } = req.body;
  const info = db
    .prepare(
      'INSERT INTO projects (title, description, cover, tags, category, github_url, demo_url, featured, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
    )
    .run(String(title), String(description), String(cover), JSON.stringify(tags), String(category), String(githubUrl), String(demoUrl), featured ? 1 : 0, new Date().toISOString());
...
router.put('/projects/:id', requireAdmin, (req, res) => {
  ...
  const b = req.body || {};
  db.prepare(
    'UPDATE projects SET title = ?, description = ?, cover = ?, tags = ?, category = ?, github_url = ?, demo_url = ?, featured = ? WHERE id = ?'
  ).run(
